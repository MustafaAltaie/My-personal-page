...
import BackendSkills from '../models/backendSkillsModel.js';

const router = express.Router();

router.post('/', async (req, res) => {
    try {
        const newSkill = new BackendSkills(req.body);
...
router.get('/', async (req, res) => {
    try {
        const backendSkills = await BackendSkills.find();
...
router.put('/:id', async (req, res) => {
    try {
        const { id } = req.params;
        const updatedSkill = await BackendSkills.findByIdAndUpdate(id, req.body, { new: true });
...
router.delete('/:id', async (req, res) => {
    try {
        const { id } = req.params;
        const deletedSkill = await BackendSkills.findByIdAndDelete(id);
...
router.put('/', async (req, res) => {
    try {
        ...
        await BackendSkills.deleteMany({});
        const insertedSkills = await BackendSkills.insertMany(updatedList);
